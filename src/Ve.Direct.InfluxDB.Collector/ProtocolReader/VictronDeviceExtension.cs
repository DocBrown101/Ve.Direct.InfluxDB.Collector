...
internal static class VictronDeviceExtension
{
    private static readonly IReadOnlyDictionary<string, string> DeviceNames =
    ...
    internal static string GetVictronDeviceNameByPid(this string productId)
    {
        if (DeviceNames.TryGetValue(productId, out var deviceName))
        {
            return $"{productId} ({deviceName})";
        }

        return $"unknown device pid: {productId}";
    }
}
